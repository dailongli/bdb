...
static void raise_error(int code, const char *fmt, ...) __attribute__((noreturn));

static void raise_error(int code, const char *fmt, ...)
{
    va_list args;
    char buf[1024];
    VALUE exc;
    ...
    // 2. 构造 Ruby 异常参数
    argv[0] = rb_str_new_cstr(buf); // Ruby 2.6 推荐用 rb_str_new_cstr
    argv[1] = INT2NUM(code);
